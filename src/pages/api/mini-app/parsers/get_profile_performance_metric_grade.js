...
export default async function handler(req, res) {
  ...
  try {
    ...
    const metricGradeUrl = process.env.METRIC_GRADE;

    const response = await fetch(metricGradeUrl, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: 'Ошибка API средний балл',
        status: response.status,
        details: error
      });
    }

    const metricgrade = await response.json();
    res.status(200).json({
      success: true,
      metricgrade
    });
